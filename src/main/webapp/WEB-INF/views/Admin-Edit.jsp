...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 账户管理 <span class="c-gray en">&gt;</span> 工作人员权限分配 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		...
		</span> <span class="r">共有数据：<strong>${EditUsersSize}</strong> 条</span>
	</div>
...
<div class="updata2" id="mainContent">
	<div method="get" action="updateMedicine" class="basic-grey">
		<label>
			<span style="width: auto">登录名：</span>
			<input id="uUsername" type="text" name="uName"  placeholder="${EditUser.uUsername}"/>
		</label>
		<label>
			<span style="width: auto">密&nbsp;&nbsp;&nbsp;&nbsp;码：</span>
			<input  id="uPassword" type="text" name="uPassword"  placeholder="${EditUser.uPassword}"/>
		</label>
		<label>
			<span style="width: auto">职&nbsp;&nbsp;&nbsp;&nbsp;务：</span>
		...
		<label style="text-align: center">
			...
			<input id="uuid" style="display: none" value="${EditUser.uId}">
...
<script type="text/javascript">
	...
	function admin_edit(obj) {
		var uId = obj.nextElementSibling.value;

		$.ajax({
			type : "POST",
			dataType : "JSON",
			data : {'EditAdminId' : uId},
			url: "${pageContext.request.contextPath}/ShowUserMsg",
			success :function (EditUser) {
				$("#mainContent").css("display","block");
				for (var key in EditUser)
				{
					if(key=="uUsername"){
						$("#uUsername").attr("value",EditUser[key]);
					}else if(key=="uPassword"){
						$("#uPassword").attr("value",EditUser[key]);
					}else if(key=="uId"){
						$("#uuid").attr("value",EditUser[key]);
					}
				}
			}
	...
	function UpDate(obj) {
	  ...
	  var ChangeLimit = $("#ChangeLimit option:selected").val();
		$.ajax({
			type : "POST",
			dataType : "text",
			data : {
